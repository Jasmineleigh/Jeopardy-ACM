...
public class Main extends Application {
    ...
    private VBox buildListOfWaitingPatients() {
        ...
        lvwWaitingPatients.setPrefHeight(575);
        lvwWaitingPatients.setPrefWidth(365);
        ...
        VBox vBoxNavButtons = new VBox();
//        vBoxNavButtons.getStyleClass().add("buttons_box");
        vBoxNavButtons.setSpacing(60);
       
 
        VBox vBoxContainer = new VBox();
        vBoxContainer.getStyleClass().add("buttons_h_box");
        HBox display = new HBox();
        HBox hBoxPatientButtons = new HBox();
        hBoxPatientButtons.getStyleClass().add("buttons_h_box");

        display.getStyleClass().add("list");
        hBoxPatientButtons.getChildren().addAll(btnNewPatient, btnPrepPatient, btnCheckIn, btnMoveUp, btnMoveDown);
        display.getChildren().addAll(lvwWaitingPatients);
    
        vBoxContainer.getChildren().addAll(hBoxPatientButtons, display);
    ...
    public void showCheckInScreen(Patient p) {
    	try {
		    ...
		    Scene scene = new Scene(grdRootPane, 240, 200);
			scene.getStylesheets().add(getClass().getResource("new_application_stylesheet.css").toExternalForm());
			primaryStage.setScene(scene);
			primaryStage.setTitle("Application");
			primaryStage.show();
